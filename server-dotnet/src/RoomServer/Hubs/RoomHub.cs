...
public class RoomHub : Hub
{
    ...
    /// <summary>
    /// Extracts the command <c>target</c> field from payloads that may arrive in a handful of
    /// serialization shapes (System.Text.Json primitives or dictionaries produced by JSON
    /// converters). This keeps the type matching localized so the rest of the hub code can stay
    /// agnostic about how the client serialized the command.
    /// Supported payload representations:
    /// <list type="bullet">
    /// <item><description><see cref="JsonElement"/> with <see cref="JsonValueKind.Object"/></description></item>
    /// <item><description><see cref="JsonDocument"/> with object root</description></item>
    /// <item><description><see cref="IDictionary{TKey, TValue}"/> values (object or string)</description></item>
    /// </list>
    /// Any other payload shapes are treated as missing the <c>target</c> field and return <c>null</c>.
    /// </summary>
